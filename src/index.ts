...
export const executeTransformation = async <T>(
  ...
  const sqlQueryElapsedTimeInSeconds =
    (sqlQueryEndTime - sqlQueryStartTime) / 1000;
  if (options?.debug) {
    console.log(
      `[executeTransformation] Executed query in ${sqlQueryElapsedTimeInSeconds.toPrecision(2)}s.`,
    );
  }
